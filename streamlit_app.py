    ...
    if not input_source:
        ...
        with st.spinner("Analyzing document..."):
            try:
                ...
                for paragraph, details in output.items():
                    with st.expander(f"{paragraph}"):
                        st.write(f"**Text:** {details['text']}")
                        st.write(f"**Action Items:** {details['actions']}")
                        st.write(f"**Risk Mitigations:** {details['mitigations']}")
